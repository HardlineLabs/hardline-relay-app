...
class ChannelProvisioningTest {
    ...
    @Test
    fun generatesDistinct256BitKeys() {
    ...
    @Test
    fun qrRoundTripPreservesOnlyOnePrivateChannel() {
        val expected = privateChannel()
        assertEquals(expected, ChannelProvisioning.decode(ChannelProvisioning.encode(expected)))
    }

    @Test
    fun generatedQrCanBeDecodedByScannerEngine() {
        val expected = privateChannel()
        val matrix =
            com.google.zxing.qrcode
                .QRCodeWriter()
                .encode(
                    ChannelProvisioning.encode(expected),
                    com.google.zxing.BarcodeFormat.QR_CODE,
                    400,
                    400,
                )
        val pixels =
            IntArray(400 * 400) { index -> if (matrix[index % 400, index / 400]) -16777216 else -1 }
    ...
    @Test
    fun rejectsInvalidNamesAndPublicKeys() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            ChannelProvisioning.validate(primary.copy(name = "Test"))
        }
        assertThrows(IllegalArgumentException::class.java) {
    ...
    @Test
    fun appendsWithoutReplacingPrimaryOrOtherChannels() {
    ...
    @Test
    fun exactImportIsIdempotentButNameConflictFails() {
        val channel = privateChannel()
        assertEquals(1, ChannelProvisioning.selectSlot(listOf(primary, channel), channel, 8))
        assertThrows(IllegalArgumentException::class.java) {
            ChannelProvisioning.selectSlot(
                listOf(primary, channel),
                ChannelProvisioning.create(channel.name),
                8,
            )
        }
        assertThrows(IllegalArgumentException::class.java) {
    ...
    @Test
    fun handlesEmptySlotsAndFullRadioSafely() {
        assertEquals(
            1,
            ChannelProvisioning.selectSlot(
                listOf(primary, ChannelSettings(), privateChannel("Other")),
                privateChannel(),
                8,
            ),
        )
        assertThrows(IllegalStateException::class.java) {
            ChannelProvisioning.selectSlot(
                listOf(primary, privateChannel("Other")),
                privateChannel(),
                2,
            )
        }
    ...
    @Test
    fun rejectsForeignMalformedAndConfigurationReplacingQr() {
        val valid = ChannelProvisioning.encode(privateChannel())
        listOf(
                valid.replace("meshtastic.org", "example.com"),
                valid.replace("https", "http"),
                valid.replace("?add=true", ""),
                "x".repeat(2049),
                "not a QR",
            )
            .forEach {
                assertThrows(IllegalArgumentException::class.java) {
                    ChannelProvisioning.decode(it)
                }
            }
        val set = ChannelSet(settings = listOf(privateChannel()), lora_config = Config.LoRaConfig())
        val replacing =
            "https://meshtastic.org/e/?add=true#" +
                Base64.getUrlEncoder().withoutPadding().encodeToString(set.encode())
