...
class MeshChannelClientTest {
            ...
            when (method.name) {
                ...
                "requestReboot" -> {
                    ...
                    if (!dropReadBack) {
                        ...
                        channels = channels.copy(settings = settings.dropLastWhile { it == ChannelSettings() })
                    }
                    null
                }
    ...
    @Test fun removalVerifiesFreshChannelSetAndPreservesUnrelatedSlots() {
        val radio = FakeRadio()
        radio.channels = radio.channels.copy(settings = radio.channels.settings + ChannelProvisioning.create("Remove") + ChannelProvisioning.create("Keep"))
        ...
        assertEquals(1, radio.reboots); assertEquals(0, radio.configWrites)
    }
    @Test fun removingPrimaryMovesOneExplicitKeyChannelToPrimary() {
        val radio = FakeRadio()
        ...
        assertEquals(listOf(Channel.Role.PRIMARY, Channel.Role.DISABLED), radio.channelWrites.map { it.role })
        assertEquals(0, radio.configWrites)
    }
    @Test fun lostRemovalReadbackAndConcurrentChangesNeverClaimSuccess() {
        val radio = FakeRadio().apply { dropReadBack = true }
        radio.channels = radio.channels.copy(settings = radio.channels.settings + ChannelProvisioning.create("Lab"))
    ...
    @Test fun solePrimaryAndInheritedKeysCannotBeRemovedAccidentally() {
        val radio = FakeRadio(); val client = MeshChannelClient(radio.service)
        assertThrows(IllegalStateException::class.java) { client.remove(0, client.read()) {} }
        radio.channels = radio.channels.copy(settings = radio.channels.settings + ChannelSettings(name = "Inherited"))
        assertThrows(IllegalStateException::class.java) { client.remove(0, client.read()) {} }
