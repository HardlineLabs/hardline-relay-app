...
window.relayUpdate = function(data){
  ...
  if(!centered && nodes.length){relayFit();centered=true;}
  message();
};
window.relayFit=function(){
  ...
  map.fitBounds(L.latLngBounds(points),{padding:[38,38],maxZoom:13,animate:false});
};
window.relayLocal=function(){const n=nodes.find(n=>n.number===local);if(n)map.setView([n.lat,n.lon],Math.max(map.getZoom(),12));};
window.relayFocus=function(id){const n=nodes.find(n=>n.id===id);if(!n)return;focused=id;map.setView([n.lat,n.lon],Math.max(map.getZoom(),14),{animate:false});markers.forEach((m,key)=>m.getElement()?.classList.toggle('focused',key===id));};
