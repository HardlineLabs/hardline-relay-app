...
class ProfileProvider : ContentProvider() {
    ...
    override fun query(uri: Uri, projection: Array<out String>?, selection: String?,
        ...
        val cursor = MatrixCursor(arrayOf("id", "name", "locked", "activation", "node", "index", "slot", "fingerprint", "switching"))
        ...
        snapshot.profiles.forEach { pkg ->
            ...
            cursor.addRow(arrayOf<Any>(pkg.id, pkg.name, if (pkg.locked) 1 else 0,
                a?.optString("activation") ?: "", a?.optInt("node") ?: 0,
                a?.optInt("index") ?: -1, a?.optInt("slot") ?: 0, a?.optString("fingerprint") ?: "", if (snapshot.switching) 1 else 0))
        }
        return cursor
    }
