...
class CongestionChart(context: Context) : LinearLayout(context) {
    ...
    private val slider =
        SeekBar(context).apply {
            contentDescription = "Inspect channel congestion samples"
            tag = "congestion-scrubber"
        }
    private val graph =
        object : View(context) {
            ...
            override fun onDraw(c: Canvas) {
                ...
                for (level in listOf(0, 50, 100)) {
                    ...
                    paint.color = Color.rgb(59, 76, 84)
                    c.drawLine(40f, y, width.toFloat(), y, paint)
                    paint.color = Color.rgb(159, 180, 190)
                    c.drawText("$level", 0f, y.coerceAtLeast(15f), paint)
                }
                visible.forEach {
                    val x =
                        40 +
                            ((it.time - (end - window)).toDouble() / window * (width - 44))
                                .toFloat()
                    val y = 10 + (100 - it.busy) / 100f * (bottom - 10)
                    paint.color =
                        if (it.busy >= 50) Color.rgb(255, 195, 100) else Color.rgb(93, 218, 196)
    ...
    init {
        ...
        choose.setOnClickListener {
            android.app.AlertDialog.Builder(context)
                .setTitle("Congestion history")
                .setItems(windows.toTypedArray()) { _, i ->
                    window = lengths[i]
                    choose.text = "History · ${windows[i]}"
                    selectedTime = null
                    following = true
                    refresh()
                }
        ...
        addView(choose)
        addView(graph, LayoutParams(-1, (130 * resources.displayMetrics.density).toInt()))
        addView(slider)
        addView(
            Button(context).apply {
                text = "Latest reading"
        ...
        slider.setOnSeekBarChangeListener(
            object : SeekBar.OnSeekBarChangeListener {
                ...
                override fun onProgressChanged(s: SeekBar, progress: Int, user: Boolean) {
                    if (user) {
                        following = false
                        selectedTime = visible.getOrNull(progress)?.time
                        describe()
                        graph.invalidate()
                    }
                }
    ...
    private fun refresh() {
        ...
        slider.max = (visible.size - 1).coerceAtLeast(0)
        slider.isEnabled = visible.size > 1
        val index =
            if (following) visible.lastIndex
            else
                visible.indexOfFirst { it.time == selectedTime }.takeIf { it >= 0 }
                    ?: visible.lastIndex
        selectedTime = visible.getOrNull(index)?.time
        slider.progress = index.coerceAtLeast(0)
        describe()
        graph.invalidate()
    }
    ...
    private fun describe() {
        ...
        detail.text =
            if (sample == null)
                "No readings in this window. Start capture; new local radio telemetry appears here. Gaps are unknown."
            else {
                val date =
                    SimpleDateFormat("MMM d HH:mm:ss", Locale.getDefault())
                        .format(Date(sample.time))
