...
internal class PacketFeed(
    ...
    init {
        ...
        val header = ui.column().apply { setPadding(ui.dp(12), 0, ui.dp(12), 0) }
        addView(header)
        count = ui.label(header, size = 13f, color = ui.muted).also(ui::single)
        val controls = ui.row()
        header.addView(controls)
        listOf("All", "TX", "RX").forEach { title ->
            ui.button(controls, title) {
                filter = title
                node = null
                expanded.clear()
                refresh(true)
        ...
        arrivals =
            ui.button(header, "Live · newest packets first") {
                    refresh(true)
                    list.setSelection(0)
                }
                .apply { tag = "packet-arrivals" }
    ...
    private fun key(e: RadioEvent) =
        "${e.time}/${e.direction}/${e.source}/${e.destination}/${e.packetId}/${e.kind}"

    private fun refresh(force: Boolean) {
        if (eventSnapshot != state.events) {
            eventSnapshot = state.events
            packets = RadioEvidence.packets(eventSnapshot)
        }
        latest =
            packets
                .filter {
                    (filter == "All" || it.direction == filter) &&
                        (node == null || it.source == node || it.destination == node)
                }
                .reversed()
        val top =
            list.firstVisiblePosition == 0 &&
                (list.getChildAt(0)?.top ?: list.paddingTop) >= list.paddingTop
        ...
        if (oldKeys != rows.map(::key)) adapter.notifyDataSetChanged()
            ...
            for (i in 0 until list.childCount) {
                ...
                if (position in rows.indices)
                    (list.getChildAt(i).tag as? PacketViews)?.bind(rows[position])
            }
        val oldKeySet = oldKeys.toSet()
        val waiting = newKeys.count { it !in oldKeySet }
        arrivals.update(
            if (!canInsert && waiting > 0) "$waiting new packets · tap to show"
            else "Live · ${node?.let { nodeName(it, state) } ?: filter} · tap for newest"
        )
        count.update(
            "${packets.count { it.direction == "TX" }} TX · ${packets.count { it.direction == "RX" }} RX · ${if (latest.isEmpty()) "waiting for packets" else "tap a packet to expand"}"
        )
    ...
    private inner class PacketAdapter : BaseAdapter() {
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
            ...
            (root.tag as PacketViews).bind(rows[position])
            return root
        }
    }
    ...
    private inner class PacketViews(root: LinearLayout) {
        ...
        val inspectButton =
            ui.button(details, "Inspect node") {
                event?.let { e ->
                    state.nodes
                        .firstOrNull {
                            it.id == if (e.direction == "TX") e.destination else e.source
                        }
                        ?.let(inspect)
                }
            }
        ...
        fun bind(e: RadioEvent) {
            ...
            heading.update(
                "${e.direction}  ·  ${clock(e.time)}  ·  ${if (open) "Collapse ▴" else "Details ▾"}"
            )
            ...
            glance.update(
                "$status · ${if (e.size < 0) "Size unknown" else "${e.size} B payload"}\n${hopsLabel(e)}${if (e.transport == "MQTT") " · MQTT" else ""}"
            )
            if (open) {
                val transitions =
                    state.events
            ...
            card.contentDescription =
                "${e.direction}, ${endpoint.text}, ${glance.text}, ${if (open) "expanded" else "collapsed"}"
        }
    }
}
