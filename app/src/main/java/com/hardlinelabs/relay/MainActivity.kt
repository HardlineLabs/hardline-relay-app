...
class MainActivity : Activity() {
    ...
    private val mapLocation =
        object : LocationListener {
            override fun onLocationChanged(location: Location) {
                if (location.isFromMockProvider) return
                RadioMonitorService.instance?.phonePosition(
                    location.latitude,
                    location.longitude,
                    location.time,
                )
    ...
    private fun refreshConnection() {
        ...
        worker.execute {
            val result = runCatching {
                ...
                current to
                    MonitorState(
                        connected = true,
                        local = current.node,
                        longName = own?.user?.longName.orEmpty(),
                        shortName = own?.user?.shortName.orEmpty(),
                        battery = metrics?.batteryLevel,
                        voltage = metrics?.voltage,
                        utilization = metrics?.channelUtilization,
                        airtime = metrics?.airUtilTx,
                        uptime = metrics?.uptimeSeconds,
                        metricTime = metrics?.time?.toLong()?.times(1000) ?: 0,
                        context =
                            "${current.config.lora?.region} · ${current.config.lora?.modem_preset} · ${current.config.lora?.hop_limit} hops",
            ...
            runOnUiThread {
                ...
                if (
                    current?.first?.node != snapshot?.node ||
                        current?.first?.channels != snapshot?.channels ||
                        current?.first?.config != snapshot?.config
                ) {
                ...
                observatory.connectionSnapshot(
                    current?.second ?: MonitorState(message = "Radio disconnected")
                )
            }
        }
    }
    ...
    private fun compatibilityScan() {
        ...
        if (api == null) {
            AlertDialog.Builder(this)
                .setTitle("Compatibility for ATAK")
                .setMessage("Radio unavailable. Open Meshtastic and connect first.")
        ...
        worker.execute {
            val report = runCatching {
                ...
                checks.add(
                    0,
                    AtakCompatibility.Finding(
                        if (version == 29320069) "OK" else "BLOCKED",
                        "Meshtastic Android must be 2.7.13; installed code $version.",
                    ),
                )
                ...
                checks.add(
                    1,
                    AtakCompatibility.Finding(
                        if (firmware == "2.7.15.567b8ea") "OK" else "BLOCKED",
                        "Radio firmware: $firmware; supported 2.7.15.567b8ea.",
                    ),
                )
                ...
                checks.add(
                    AtakCompatibility.Finding(
                        if (age == null || age !in 0..900 || busy == null || busy >= 50) "CHECK"
                        else "OK",
                ...
                checks.add(
                    AtakCompatibility.Finding(
                        if (gps) "OK" else "BLOCKED",
                        "ATAK precise-location permission ${if (gps) "granted; a fresh GPS fix is still needed" else "missing"}.",
                    )
                )
                ...
                val verified =
                    active != null &&
                        active.optInt("node") == current.node &&
                        index > 0 &&
                        current.config.lora?.channel_num == active.optInt("slot") &&
                        current.config.lora?.override_frequency == 0f &&
                        channel?.let { ChannelProfile.digest(it.encode()) } ==
                            active.optString("fingerprint")
                checks.add(
                    AtakCompatibility.Finding(
                        if (!verified || channel == null) "CHECK"
                        else if (
                            channel.psk.size != 32 ||
                        ...
                        if (!verified || channel == null)
                            "Saved activation does not match the live radio/channel settings. Activate and verify the intended profile in Relay."
                        else
                            "Active private channel: ${channel.name}. Requires a 32-byte PSK and MQTT uplink/downlink off; teammate key equality cannot be checked locally.",
                    )
                )
                if (profile.switching)
                    checks.add(
                        AtakCompatibility.Finding(
                            "BLOCKED",
                            "Channel activation needs verification in Relay.",
                        )
                    )
    ...
    private fun button(parent: LinearLayout, label: String, action: () -> Unit): Button =
        Button(this).apply {
            ...
            minHeight = dp(48)
            setTextColor(ink)
            backgroundTintList = android.content.res.ColorStateList.valueOf(Color.rgb(44, 67, 73))
            isEnabled = !busy
    ...
    private fun input(hintText: String, secret: Boolean = false) =
        EditText(this).apply {
            hint = hintText
            isSingleLine = true
            setTextColor(ink)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        window.statusBarColor = Color.rgb(13, 21, 27)
        window.navigationBarColor = Color.rgb(13, 21, 27)
        store = ProfileStore(this)
        // The plugin runs as ATAK's UID; its own manifest cannot change host visibility.
        runCatching {
            grantUriPermission(
                "com.atakmap.app.civ",
                android.net.Uri.parse("content://com.hardlinelabs.relay.profiles/profiles"),
        ...
        val layout =
            column().apply {
                ...
                setBackgroundColor(Color.rgb(13, 21, 27))
            }
        ...
        button(connectionCard, "Refresh radio") {
            if (service == null) {
                disconnect()
                bindMesh()
            } else refresh()
        ...
        button(actions, "Scan channel QR") {
            IntentIntegrator(this)
                ...
                .setPrompt("Scan a Hardline channel")
        ...
        radioList = column()
        layout.addView(radioList)
        button(layout, "Open Meshtastic") {
            packageManager.getLaunchIntentForPackage("com.geeksville.mesh")?.let(::startActivity)
        }
        val atak =
    ...
    override fun onResume() {
        super.onResume()
        connectionHandler.removeCallbacks(connectionTick)
        connectionHandler.post(connectionTick)
        if (::observatory.isInitialized) observatory.resume()
        if (mapGpsRequested) usePhoneGps()
    ...
    override fun onPause() {
        connectionHandler.removeCallbacks(connectionTick)
        if (::observatory.isInitialized) observatory.pause()
        getSystemService(LocationManager::class.java).removeUpdates(mapLocation)
        super.onPause()
    }
    ...
    override fun onDestroy() {
        connectionHandler.removeCallbacks(connectionTick)
        if (::observatory.isInitialized) observatory.destroy()
        disconnect()
    ...
    override fun onBackPressed() {
        if (!observatory.back()) super.onBackPressed()
    }
    ...
    private fun startCapture() {
        ...
        if (required.isNotEmpty()) {
            requestPermissions(required.toTypedArray(), 51)
            return
        }
    ...
    private fun usePhoneGps() {
        ...
        getSystemService(LocationManager::class.java)
            .requestLocationUpdates(LocationManager.GPS_PROVIDER, 10_000, 5f, mapLocation)
        Toast.makeText(
                this,
                "Waiting for phone GPS. Used only for the local map; never transmitted.",
                Toast.LENGTH_LONG,
            )
            .show()
    }
    ...
    private val connection =
        object : ServiceConnection {
            ...
            override fun onServiceDisconnected(name: ComponentName) {
                service = null
                snapshot = null
                status.text = "Radio unavailable. Reconnect in Meshtastic."
                renderRadio()
            }
            ...
            override fun onBindingDied(name: ComponentName) {
                disconnect()
                snapshot = null
                status.text = "Connection lost. Tap Refresh radio."
            }
    ...
    private fun bindMesh() {
        ...
        val version = runCatching {
            packageManager.getPackageInfo("com.geeksville.mesh", 0).versionCode
        }
            .getOrNull()
        if (version != 29320069) {
            status.text =
                "Install Meshtastic 2.7.13 to connect a radio. Saved channels work offline."
            return
        }
        bound =
            runCatching {
    ...
    private fun task(message: String, showProgress: Boolean = false, work: () -> String) {
        ...
        worker.execute {
            ...
            runOnUiThread {
                ...
                status.text = result.getOrElse {
                    it.message ?: "Operation interrupted. Refresh and verify the radio."
                }
                if (activationResult)
                    AlertDialog.Builder(this)
                        .setTitle(
                            if (result.isSuccess) "Channel active" else "Activation not completed"
                        )
                ...
                if (result.isSuccess)
                    pendingId?.let { id ->
                        pendingId = null
                        runCatching { store.list().firstOrNull { it.id == id } }
                            .getOrNull()
                            ?.let(::activateDialog)
                    }
            }
    ...
    private fun refresh() {
        ...
        task("Checking radio…") {
            snapshot = null
            val current = MeshChannelClient(s).read()
    ...
    private fun createDialog() {
        val current =
            snapshot
                ?: run {
                    status.text = "Connect and refresh the radio before creating a channel."
                    return
                }
        val layout = column().apply { setPadding(dp(20), dp(8), dp(20), 0) }
        val formError =
            text(layout, "", 13f, Color.rgb(255, 151, 141)).apply { visibility = View.GONE }
        val name = input("Channel name (1–11 characters)")
        layout.addView(name)
        val publicMesh =
            Switch(this).apply {
                text = "Public-mesh compatible"
                isChecked = true
            }
        layout.addView(publicMesh)
        text(
            layout,
            "Both modes are encrypted. Separate frequency switches the whole radio; teammates must switch too.",
            13f,
            muted,
        )
        val protect = Switch(this).apply { text = "Protect with a passphrase" }
        layout.addView(protect)
        val password = input("Passphrase (12+ characters)", true)
        layout.addView(password)
        val repeat = input("Repeat passphrase", true)
        layout.addView(repeat)
        password.visibility = View.GONE
        repeat.visibility = View.GONE
        protect.setOnCheckedChangeListener { _, checked ->
            password.visibility = if (checked) View.VISIBLE else View.GONE
            repeat.visibility = password.visibility
        }
        val dialog =
            AlertDialog.Builder(this)
                ...
                .setPositiveButton("Save channel", null)
                .create()
        dialog.setOnShowListener {
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                try {
                    ...
                    val pass =
                        if (protect.isChecked) password.text.toString().toCharArray() else null
                    val repeated = repeat.text.toString().toCharArray()
                    val valid =
                        pass == null || (pass.size in 12..128 && pass.contentEquals(repeated))
                    repeated.fill('\u0000')
                    if (!valid) {
                        pass?.fill('\u0000')
                        password.error = "Use matching passphrases of 12–128 characters."
                        return@setOnClickListener
                    }
                    val slot = ChannelProfile.newSlot(publicMesh.isChecked)
                    task("Saving encrypted channel…") {
                        try {
                            store.save(ChannelProfile.create(settings, slot, pass))
                            "${settings.name} saved${if (pass != null) " · Locked" else ""}."
                        } finally {
                            pass?.fill('\u0000')
                        }
                    }
                    ...
                    formError.text = e.message ?: "Check channel details."
                }
            }
        }
    ...
    private fun activateDialog(pkg: ChannelProfile.Package) {
        val current =
            snapshot
                ?: run {
                    status.text = "Connect the radio before activating ${pkg.name}."
                    return
                }
        if (SystemClock.elapsedRealtime() < unlockAfter) {
            status.text = "Wait a few seconds before trying again."
            return
        }
        val layout = column().apply { setPadding(dp(20), dp(8), dp(20), 0) }
        text(
            layout,
            "Activation installs this key on the radio. A different frequency leaves your current mesh.",
        )
        ...
        dialog.setOnShowListener {
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                ...
                task("Unlocking ${pkg.name}…", showProgress = true) {
                    val opened =
                        try {
                            ChannelProfile.open(pkg, pass)
                        } catch (e: Exception) {
                            unlockAfter = SystemClock.elapsedRealtime() + 5000
                            throw e
                        } finally {
                            pass?.fill('\u0000')
                        }
                    ...
                    for (seconds in 6 downTo 1) {
                        progress(
                            "Activating ${pkg.name} · waiting ${seconds}s for plugin traffic to pause…"
                        )
                        Thread.sleep(1000)
                    }
                    val (index, after) =
                        MeshChannelClient(s).activate(opened, current.node, ::progress)
                    snapshot = after
                    store.activated(pkg, opened, after.node, index)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
                    ...
                    worker.execute {
                        val result = runCatching {
                            contentResolver.openOutputStream(uri, "wt")!!.bufferedWriter().use { out
                                ->
                                events.forEach {
                                    out.write(ObservationStore.encode(it).toString())
                                    out.newLine()
                                }
                            }
                        }
                        runOnUiThread {
                            Toast.makeText(
                                    this,
                                    if (result.isSuccess) "Metadata exported" else "Export failed",
                                    Toast.LENGTH_LONG,
                                )
                                .show()
                        }
        ...
        val content =
            IntentIntegrator.parseActivityResult(requestCode, resultCode, data)?.contents ?: return
    ...
    private fun clearList(list: LinearLayout) {
        controls.removeAll { button ->
            var p = button.parent
            while (p is View && p !== list) p = p.parent
            p === list
        }
        list.removeAllViews()
    }
    ...
    private fun renderSaved() {
        ...
        try {
            ...
            if (packages.isEmpty())
                text(
                    savedList,
                    "Create or scan a channel to prepare your contingencies.",
                    14f,
                    muted,
                )
            ...
            packages.forEach { pkg ->
                ...
                text(
                    c,
                    if (snapshot == null) "Saved · Radio state unverified"
                    else if (installed) "Key installed on radio"
                    else if (pkg.locked) "Locked · Not installed" else "Saved · Not installed",
                    13f,
                    muted,
                )
                val current = snapshot
                val isActive =
                    active?.optString("id") == pkg.id &&
                        current != null &&
                        active.optInt("node") == current.node &&
                        current.config.lora?.channel_num == active.optInt("slot") &&
                        current.config.lora?.override_frequency == 0f &&
                        current.channels.settings.getOrNull(active.optInt("index"))?.let {
                            ChannelProfile.digest(it.encode())
                        } == active.optString("fingerprint")
                if (isActive)
                    text(
                        c,
                        if (active!!.optInt("slot") == 20) "Active · Public-mesh compatible"
                        else "Active · Separate frequency",
                        13f,
                        accent,
                    )
                val actions = LinearLayout(this)
                c.addView(actions)
                button(actions, if (pkg.locked && !installed) "Unlock" else "Activate") {
                        activateDialog(pkg)
                    }
                    .contentDescription =
                    if (pkg.locked && !installed) "Unlock & activate ${pkg.name}"
                    else "Activate ${pkg.name}"
                button(actions, "Share QR") { showQr(pkg) }.contentDescription =
                    "Share QR: ${pkg.name}"
                button(actions, "Remove profile") {
                        AlertDialog.Builder(this)
                            .setTitle("Remove saved ${pkg.name}?")
                            .setMessage(
                                "Removes this profile from the app. Installed radio channels remain unchanged."
                            )
                            .setNegativeButton("Cancel", null)
                            .setPositiveButton("Remove profile") { _, _ ->
                                task("Removing saved profile…") {
                                    store.remove(pkg.id)
                                    "Saved profile removed. Radio channels unchanged."
                                }
                            }
                            .show()
                    }
            ...
            text(
                savedList,
                "Saved channels cannot be unlocked on this phone. Existing data has been preserved.",
                15f,
                Color.rgb(255, 151, 141),
            )
        }
    }
    ...
    private fun renderRadio() {
        ...
        val current =
            snapshot
                ?: run {
                    text(radioList, "Radio unavailable", 14f, muted)
                    return
                }
        current.channels.settings.forEachIndexed { index, c ->
            if (index == 0 || c != org.meshtastic.proto.ChannelSettings()) {
                val row = card(radioList)
                text(row, radioChannelName(current, index), 18f)
                ...
                if (index > 0 && runCatching { ChannelProvisioning.validate(c) }.isSuccess) {
                    button(row, "Send test: ${c.name}") {
                        ...
                        if (SystemClock.elapsedRealtime() - lastTest < 5000) {
                            status.text = "Wait five seconds between tests."
                            return@button
                        }
    ...
    private fun radioChannelName(current: MeshChannelClient.Snapshot, index: Int) =
        current.channels.settings[index].name.ifBlank {
            current.config.lora?.modem_preset?.name?.replace('_', ' ') ?: "Unnamed channel"
        }
    ...
    private fun removeRadioDialog(index: Int, current: MeshChannelClient.Snapshot) {
        ...
        val replacement =
            if (index == 0)
                current.channels.settings.indices.firstOrNull {
                    it > 0 &&
                        current.channels.settings[it] != org.meshtastic.proto.ChannelSettings()
                }
            else null
        if (index == 0 && replacement == null) {
            status.text = "Install another channel before removing the primary channel."
            return
        }
        val explanation =
            "Remove $name from radio slot $index? The radio will restart briefly so Relay can verify removal." +
                (replacement?.let {
        ...
        AlertDialog.Builder(this)
            ...
            .setNegativeButton("Cancel", null)
            .setPositiveButton("Remove from radio") { _, _ ->
    ...
    private fun showQr(pkg: ChannelProfile.Package) {
        ...
        val image =
            ImageView(this).apply {
                ...
                contentDescription = "Channel provisioning QR"
            }
        secretDialog =
            AlertDialog.Builder(this)
                .setTitle(pkg.name)
                .setMessage(
                    if (pkg.locked) "Scan in Hardline Relay. Share the passphrase separately."
                    else "This QR contains the channel key. Share only with teammates."
                )
                .setView(image)
                .setPositiveButton("Done", null)
                .create()
