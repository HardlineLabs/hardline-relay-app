...
class ProfileStore(private val context: Context, private val fileName: String = "channels.v1") {
    companion object {
        ...
        private const val ALIAS = "hardline.profiles.v1"
    }

    private val file
        get() = AtomicFile(File(context.noBackupFilesDir, fileName))
    ...
    data class Snapshot(
        val profiles: List<ChannelProfile.Package>,
    ...
    fun snapshot(): Snapshot =
        synchronized(lock) {
            ...
            Snapshot(
                (0 until array.length()).map { ChannelProfile.parse(array.getString(it)) },
                root.optJSONObject("active"),
                root.optBoolean("switching", false),
            )
        }

    fun selectRadio(node: Int) =
        synchronized(lock) {
            val root = read()
            if (
                root.optInt("selectedRadio", root.optJSONObject("active")?.optInt("node") ?: 0) ==
                    node
            )
                return@synchronized
            val selected =
            ...
            if (selected != 0)
                radios.put(
                    selected.toString(),
                    JSONObject()
                        .put("active", root.optJSONObject("active"))
    ...
    fun save(pkg: ChannelProfile.Package) =
        synchronized(lock) {
            val root = read()
            val existing = root.getJSONArray("profiles")
            val packages =
                (0 until existing.length()).map { ChannelProfile.parse(existing.getString(it)) }
            if (packages.any { it.id == pkg.id }) return@synchronized
            require(packages.none { it.name.equals(pkg.name, true) }) {
                "A saved channel already uses this name. Nothing overwritten."
            }
            require(packages.size < 64) {
                "Saved channel limit reached. Remove an unused profile first."
            }
            existing.put(pkg.qr())
            write(root)
        }
    ...
    fun remove(id: String) =
        synchronized(lock) {
            ...
            root.optJSONObject("radios")?.let { radios ->
                radios.keys().forEach { radio ->
                    radios.optJSONObject(radio)?.let { state ->
                        if (state.optJSONObject("active")?.optString("id") == id)
                            state.remove("active")
                    }
                }
            }
    ...
    fun activated(pkg: ChannelProfile.Package, opened: ChannelProfile.Open, node: Int, index: Int) =
        synchronized(lock) {
            val root = read()
    ...
    fun clearActive() =
        synchronized(lock) {
            val root = read()
            root.remove("active")
            root.put("switching", true)
            write(root)
        }

    fun removalVerified(ids: Set<String>) =
        synchronized(lock) {
            val root = read()
            val array = root.getJSONArray("profiles")
            root.put(
