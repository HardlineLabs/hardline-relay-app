...
class TrafficChart(context: Context, events: List<RadioEvent> = emptyList()) : View(context) {
    ...
    init {
        contentDescription =
            "Ten-minute packet chart. Fixed scale: 10 packets per 20-second bucket; taller counts capped. Mint RX, blue TX."
    }
    ...
    override fun onDraw(c: Canvas) {
        ...
        for (i in 0..29) {
            ...
            c.drawRect(
                i * unit + 2,
                height - 24 - rx[i].coerceAtMost(10) / 10f * (height - 36),
                i * unit + unit / 2,
                height - 24f,
                paint,
            )
            ...
            c.drawRect(
                i * unit + unit / 2,
                height - 24 - tx[i].coerceAtMost(10) / 10f * (height - 36),
                (i + 1) * unit - 2,
                height - 24f,
                paint,
            )
