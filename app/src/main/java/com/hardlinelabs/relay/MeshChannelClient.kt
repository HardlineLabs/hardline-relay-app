...
class MeshChannelClient(
    private val service: IMeshService,
    private val readBackTimeoutMillis: Long = 20_000,
    ...
    fun read(): Snapshot {
        check(service.connectionState() == "Connected") {
            "Radio disconnected. Open Meshtastic and reconnect."
        }
        val info =
            service.myNodeInfo
                ?: error("Radio details unavailable. Wait for Meshtastic to finish connecting.")
        check(info.firmwareVersion == "2.7.15.567b8ea") {
            "Requires radio firmware 2.7.15.567b8ea."
        }
        return Snapshot(
            info.myNodeNum,
            info.maxChannels,
            ChannelSet.ADAPTER.decode(service.channelSet),
            LocalConfig.ADAPTER.decode(service.config),
        )
    }
    ...
    fun remove(index: Int, expected: Snapshot, progress: (String) -> Unit): Snapshot {
        val before = read()
        check(
            before.node == expected.node &&
                before.channels == expected.channels &&
                before.config == expected.config
        ) {
        ...
        val replacement =
            if (index == 0)
                before.channels.settings.indices.firstOrNull {
                    it > 0 && before.channels.settings[it] != ChannelSettings()
                } ?: error("Install another channel before removing the primary channel.")
            else null
        if (index == 0)
            check(
                before.channels.settings
                    .drop(1)
                    .filter { it != ChannelSettings() }
                    .none { it.psk.size == 0 }
            ) {
        ...
        if (replacement != null) {
            ...
            service.setChannel(
                Channel(index = 0, role = Channel.Role.PRIMARY, settings = wanted[0]).encode()
            )
        }
        val disabled = replacement ?: index
        wanted[disabled] = ChannelSettings()
        service.setChannel(
            Channel(index = disabled, role = Channel.Role.DISABLED, settings = ChannelSettings())
                .encode()
        )
        ...
        fun trimEmpty(settings: List<ChannelSettings>) = settings.dropLastWhile {
            it == ChannelSettings()
        }
        while (System.nanoTime() < deadline) {
            ...
            val after = read()
            check(after.node == before.node) {
                "Connected radio changed during removal. Inspect both radios."
            }
            check(after.config == before.config) {
                "Radio configuration changed during removal. Inspect in Meshtastic."
            }
            if (trimEmpty(after.channels.settings) == trimEmpty(wanted)) return after
        }
        error(
            "Removal unconfirmed. Reconnect and refresh before retrying. Saved profiles are retained; sending remains paused."
        )
    }
    ...
    fun add(
        ...
        timeoutMillis: Long = readBackTimeoutMillis,
    ): Pair<Int, Snapshot> {
        val before = read()
        check(before.node == expectedNode) {
            "Connected radio changed since confirmation. Refresh before retrying."
        }
        val index =
            ChannelProvisioning.selectSlot(before.channels.settings, settings, before.maxChannels)
        ...
        if (before.channels.settings.getOrNull(index) != settings) {
            service.setChannel(
                Channel(index = index, role = Channel.Role.SECONDARY, settings = settings).encode()
            )
        }
        var requested = false
        if (service.connectionState() == "Connected") {
        ...
        while (System.nanoTime() < deadline) {
            ...
            if (service.connectionState() != "Connected") {
                requested = false
                progress("Radio reconnecting after channel write…")
                continue
            }
            if (!requested) {
                service.getRemoteChannel(service.packetId, before.node, index)
                requested = true
                progress("Radio connected · Verifying installed key…")
                continue
            }
            val after = read()
            check(after.node == before.node) {
                "Connected radio changed. Stop and inspect both radios."
            }
            check(after.config == before.config) {
                "Radio configuration changed concurrently. Inspect in Meshtastic."
            }
            before.channels.settings.forEachIndexed { slot, original ->
                if (slot != index)
                    check(after.channels.settings.getOrNull(slot) == original) {
        ...
        error(
            "Write sent, but radio read-back unconfirmed. Refresh before retrying; nothing was rolled back."
        )
    }

    fun activate(
        profile: ChannelProfile.Open,
        expectedNode: Int,
        progress: (String) -> Unit,
    ): Pair<Int, Snapshot> {
        ...
        while (System.nanoTime() < deadline) {
            ...
            if (service.connectionState() != "Connected") {
                requested = false
                progress("Radio reconnecting · Keep the radio powered and nearby…")
                continue
            }
            if (!requested) {
                service.getRemoteConfig(
                    service.packetId,
                    expectedNode,
                ...
                requested = true
                progress("Radio connected · Verifying channel and frequency…")
                continue
            }
            val after = read()
            check(after.node == expectedNode) {
                "Connected radio changed during activation. Inspect both radios."
            }
            ...
            check(after.config.copy(lora = before.config.lora) == before.config) {
                "Other radio settings changed during activation."
            }
            if (after.config.lora == expectedLora) return index to after
        }
        error(
            "Activation unconfirmed. Settings may have changed. Reconnect in Meshtastic, then activate again to verify. No automatic rollback."
        )
    }
    ...
    fun send(index: Int, expected: ChannelSettings, expectedNode: Int): String {
        val current = read()
        check(
            current.node == expectedNode &&
                index > 0 &&
                current.channels.settings.getOrNull(index) == expected
        ) {
        ...
        val packet =
            DataPacket(DataPacket.ID_BROADCAST, index, "Hardline Relay test $token").apply {
