...
object ChannelProfile {
    ...
    class Package(val name: String, val locked: Boolean, val bytes: ByteArray) {
        val id: String get() = digest(bytes)
    ...
    fun validateRadio(lora: Config.LoRaConfig) {
        require(lora.region == Config.LoRaConfig.RegionCode.US && lora.use_preset &&
    ...
    fun newSlot(publicMesh: Boolean): Int {
        ...
        var slot: Int
        do { slot = random.nextInt(104) + 1 } while (slot == 20)
        return slot
    }
    fun create(settings: ChannelSettings, slot: Int, password: CharArray?): Package {
        ...
        if (password != null) require(password.size in 12..128) { "Use a passphrase of 12–128 characters." }
        val name = settings.name.toByteArray(Charsets.US_ASCII)
        val header = ByteBuffer.allocate(2 + name.size).put(if (password == null) 0 else 1)
            .put(name.size.toByte()).put(name).array()
        val plain = ByteBuffer.allocate(4 + settings.encode().size).putInt(slot).put(settings.encode()).array()
        return try {
            val payload = if (password == null) plain.copyOf() else {
    ...
    fun parse(qr: String): Package {
        ...
        val bytes = try { Base64.getUrlDecoder().decode(qr.removePrefix(PREFIX)) }
            catch (_: IllegalArgumentException) { error("Invalid channel QR.") }
    ...
    fun open(pkg: Package, password: CharArray?): Open {
        ...
        val plain = if (!pkg.locked) pkg.bytes.copyOfRange(start, pkg.bytes.size) else {
            require(password != null && password.size in 1..128) { "Enter the channel passphrase." }
            try { crypt(Cipher.DECRYPT_MODE, password, pkg.bytes.copyOfRange(start, start + 16),
                pkg.bytes.copyOfRange(start + 16, start + 28), pkg.bytes.copyOfRange(0, start),
                pkg.bytes.copyOfRange(start + 28, pkg.bytes.size)) }
            catch (_: Exception) { error("Incorrect passphrase or damaged channel package.") }
        }
        return try {
    ...
    private fun crypt(mode: Int, password: CharArray, salt: ByteArray, iv: ByteArray,
                      header: ByteArray, input: ByteArray): ByteArray {
