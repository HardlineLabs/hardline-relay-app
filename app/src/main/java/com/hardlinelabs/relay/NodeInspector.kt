...
internal class NodeInspector(
    ...
    private val showMap: (RadioNode) -> Unit = {},
    ...
    init {
        ...
        setPadding(ui.dp(16), ui.dp(8), ui.dp(16), 0)
        tag = "node-inspector"
        val header = ui.row()
        addView(header)
        ui.button(header, "‹ Back", close)
        mapButton =
            ui.button(header, "Show on map") { node?.let(showMap) }.apply { tag = "node-map" }
        title =
            ui.label(this, size = 23f).apply {
        ...
        val controls = ui.row()
        suite.addView(controls)
        test =
            ui.button(controls, "Test · 4 checks") {
                    requested = true
                    requestAt = SystemClock.elapsedRealtime()
                    test.isEnabled = false
                    summary.update("Starting addressed checks…")
                    start(number)
                }
                .apply { tag = "node-test" }
        stopButton = ui.button(controls, "Stop", stop).apply { tag = "node-stop" }
        progress =
            ProgressBar(context, null, android.R.attr.progressBarStyleHorizontal).apply { max = 4 }
        suite.addView(progress, LayoutParams(-1, ui.dp(12)))
        repeat(4) {
            checks.add(ui.label(suite, "${it + 1} · Not started", 13f).apply { ui.single(this) })
        }
        summary =
            ui.label(suite, size = 12f, color = ui.muted).apply {
                minLines = 3
                maxLines = 4
            }
        val scroll =
        ...
        ui.label(
            content,
            "Signal describes the final reception. Routing acknowledgments do not identify the destination; observed traffic is separate evidence. Tests never send public chat.",
            12f,
            ui.muted,
        )
    }
    ...
    fun update(s: MonitorState) {
        ...
        if (requested && (here && s.surveying || SystemClock.elapsedRealtime() - requestAt > 1500))
            requested = false
        test.available(
            n != null && s.connected && !s.surveying && !requested && n.number != s.local
        )
        ...
        checks.forEachIndexed { i, label ->
            ...
            val elapsed =
                a?.elapsed
                    ?: a?.takeIf { it.outcome == "Awaiting acknowledgment" }
                        ?.let { SystemClock.elapsedRealtime() - it.sent }
            label.update(
                "${i + 1} · ${a?.outcome ?: "Not started"}${elapsed?.let { " · %.0fs".format(Locale.US, it / 1000.0) } ?: ""}"
            )
        }
        val rx =
            s.events.filter {
        ...
        if (!requested)
            summary.update(
                when {
                    ...
                    here ->
                        "${attempts.count { it.outcome == "Acknowledged" }} destination · ${attempts.count { it.outcome == "Relay acknowledgment" }} routing acknowledgments · $heard RF receptions.\n" +
                            when {
                                s.surveyText.startsWith("Wait") -> s.surveyText
                                s.surveyStopReason != null ->
                                    "${s.surveyStopReason}. ${attempts.size} checks submitted; no more will start."
                                else ->
                                    "${attempts.size} checks finished. ${if (attempts.none { it.outcome == "Acknowledged" }) "Destination confirmation was not exposed." else "Future delivery is not guaranteed."}"
                            }
                    else -> s.surveyText
                }
            )
