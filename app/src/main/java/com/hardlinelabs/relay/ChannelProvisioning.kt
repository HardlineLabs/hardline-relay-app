...
object ChannelProvisioning {
    ...
    fun create(name: String): ChannelSettings {
        ...
        return ChannelSettings(name = name, psk = bytes.toByteString(),
            uplink_enabled = false, downlink_enabled = false).also { bytes.fill(0) }
    }
    ...
    fun validate(settings: ChannelSettings) {
        ...
        require(!settings.uplink_enabled && !settings.downlink_enabled) { "MQTT channel forwarding is not supported here." }
    ...
    fun encode(settings: ChannelSettings): String {
        validate(settings)
        return "https://meshtastic.org/e/?add=true#" + Base64.getUrlEncoder().withoutPadding()
    ...
    fun decode(value: String): ChannelSettings {
        ...
        return try {
            val uri = URI(value)
            require(uri.scheme == "https" && uri.host == "meshtastic.org" &&
                uri.path == "/e/" && uri.rawQuery == "add=true" && uri.port == -1 && uri.userInfo == null)
            val set = ChannelSet.ADAPTER.decode(Base64.getUrlDecoder().decode(uri.rawFragment))
            require(set.settings.size == 1 && set.lora_config == null && set.unknownFields.size == 0)
            set.settings.single().also(::validate)
        } catch (_: Exception) {
            throw IllegalArgumentException("Use a single private-channel QR created by Relay. No settings were changed.")
        }
    }

    fun selectSlot(existing: List<ChannelSettings>, incoming: ChannelSettings, maxChannels: Int): Int {
        validate(incoming)
        ...
        } ?: throw IllegalStateException("No free secondary channel. Remove one deliberately in Meshtastic first.")
    }
}
