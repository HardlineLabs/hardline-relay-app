...
internal class RadioUi(val context: Context) {
    ...
    fun label(parent: LinearLayout, value: String = "", size: Float = 14f, color: Int = ink) =
    ...
    fun card(parent: LinearLayout, title: String? = null) =
        column().apply {
            setPadding(dp(14), dp(10), dp(14), dp(10))
            background =
                GradientDrawable().apply {
                    setColor(Color.rgb(24, 36, 44))
    ...
    fun button(parent: LinearLayout, title: String, action: () -> Unit) =
        Button(context).apply {
            ...
            minHeight = dp(48)
            setTextColor(ink)
            backgroundTintList = android.content.res.ColorStateList.valueOf(Color.rgb(43, 64, 74))
            setOnClickListener { action() }
            parent.addView(
                this,
                if (parent.orientation == LinearLayout.HORIZONTAL)
                    LinearLayout.LayoutParams(0, dp(48), 1f)
                else LinearLayout.LayoutParams(-1, dp(48)),
            )
        }
    ...
    fun single(text: TextView) {
        text.maxLines = 1
        text.ellipsize = TextUtils.TruncateAt.END
    }
}

internal fun TextView.update(value: String) {
    if (text.toString() != value) text = value
}

internal fun Button.available(value: Boolean) {
    isEnabled = value
    alpha = if (value) 1f else .45f
}

internal fun nodeName(id: String, s: MonitorState): String =
    when (id) {
        "^all",
        "!ffffffff" -> "Broadcast"
        "^local" -> "This radio"
...
internal fun distance(meters: Double?) =
    meters?.let { if (it < 1000) "≈${it.toInt()} m" else "≈%.1f km".format(Locale.US, it / 1000) }
        ?: "distance unknown"

internal fun nodeEvidence(n: RadioNode): String =
    when {
        n.acknowledged > 0 && System.currentTimeMillis() - n.acknowledged in 0..600_000 ->
            "Destination acknowledgment · ${age(n.acknowledged)}"
...
internal fun positionEvidence(n: RadioNode): String {
    ...
    val precision =
        n.positionPrecision?.let {
            if (it < 32) " · coarse ($it bits)" else " · full coordinate precision"
        } ?: ""
