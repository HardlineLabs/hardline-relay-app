...
class ObservationStorageTest {
    @Test
    fun onlyMetadataSurvivesReopeningAndExpiration() {
        ...
        try {
            ...
            val event =
                RadioEvent(
                    ...
                    size = 42,
                    snr = 0f,
                    context = "US / LONG_FAST",
                    note = "Signal describes final reception",
                )
            ...
            ObservationStore(context, name).use {
                ...
                assertFalse(json.has("bytes"))
                assertFalse(json.has("psk"))
                assertEquals(0.0, json.getDouble("snr"), 0.0)
                it.clear()
                assertTrue(it.read().isEmpty())
            }
        } finally {
            context.deleteDatabase(name)
        }
    ...
    @Test
    fun radiosAndSurveyPositionsRemainIsolatedAcrossRestarts() {
        val context = ApplicationProvider.getApplicationContext<Context>()
        ...
        try {
            val now = System.currentTimeMillis()
            val own =
                com.hardlinelabs.relay.core.RadioNode(
                    1,
                    "!00000001",
                    "Origin",
                    0,
            ...
            val heard =
                own.copy(number = 2, id = "!00000002", latitude = 40.1, observed = now, hops = 3)
            val record =
                SurveyRecord(
                    "one",
                    now,
            ...
            ObservationStore(context, name).use {
                ...
                val restored =
                    SurveyRecord.read(it.snapshot()!!.getJSONArray("surveys").getJSONObject(0))
    ...
    @Test
    fun fullSurveyHistoryUsesBoundedRowsAndEvictsOldRadios() {
        val context = ApplicationProvider.getApplicationContext<Context>()
        ...
        try {
            ...
            ObservationStore(context, name).use { store ->
                ...
                store.saveSnapshot(
                    org.json.JSONObject().put("surveys", org.json.JSONArray(records))
                )
                assertEquals(20, store.snapshot()!!.getJSONArray("surveys").length())
                assertEquals(
                    1000,
                    SurveyRecord.read(store.snapshot()!!.getJSONArray("surveys").getJSONObject(0))
                        .nodes
                        .size,
                )
                for (node in 2..9) {
                    Thread.sleep(2)
                    store.radio = node
                    store.saveSnapshot(org.json.JSONObject())
                }
                store.radio = 1
