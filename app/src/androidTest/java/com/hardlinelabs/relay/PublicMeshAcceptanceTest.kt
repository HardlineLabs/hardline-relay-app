...
class PublicMeshAcceptanceTest {
    @Test
    fun observeAndOptionallySurveyOneRadio() {
        ...
        while (
            RadioMonitorService.instance?.state?.connected != true &&
        ...
        if (args.getString("survey") == "true") {
            ...
            if (args.getString("stopAfterFirst") == "true") {
                ...
                while (
                    monitor.state.attempts.isEmpty() &&
                        SystemClock.elapsedRealtime() < firstDeadline
                ) Thread.sleep(500)
                assertTrue(
                    "A check must start before testing Stop",
                    monitor.state.attempts.isNotEmpty(),
                )
                monitor.stopSurvey()
                Thread.sleep(2000)
                ...
                Thread.sleep(20_000)
                assertEquals(
                    "Stop must prevent new diagnostic submissions",
                    count,
                    monitor.state.attempts.size,
                )
            ...
            assertTrue(
                "At least one diagnostic must be attempted",
                monitor.state.attempts.isNotEmpty(),
            )
    ...
    @Test
    fun immediateNextNodeAndActiveSurveyHistory() {
        val args = InstrumentationRegistry.getArguments()
        ...
        try {
            ...
            monitor.stopSurvey()
            waitUntil(3000) { !monitor.state.surveying }
