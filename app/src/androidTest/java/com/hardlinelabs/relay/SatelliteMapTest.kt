...
class SatelliteMapTest {
    @Test
    fun discoveryHasSeparateHistoryColorsAndFreshOnlyNodes() {
        ActivityScenario.launch(MainActivity::class.java).use { scenario ->
            ...
            val unlocated =
                fresh.copy(number = 5, id = "!00000005", latitude = null, longitude = null)
            val discovery =
                SurveyRecord(
                    "discovery",
                    now,
                    context = "US",
                    nodes = listOf(known, fresh, unlocated),
                    discovery = true,
                    newNodes = listOf(3, 5),
                    requests = 1,
                )
            ...
            val state =
                MonitorState(
                    local = 1,
                    connected = true,
                    nodes = listOf(known, fresh, cache, unlocated),
                    surveys = listOf(survey, restored),
            ...
            scenario.onActivity { a ->
                ...
                web = panel.findViewWithTag("satellite-map")
                panel.update(state)
            }
            val deadline = System.currentTimeMillis() + 20000
            while (
                js(web, "typeof relayUpdate") != "\"function\"" &&
            ...
            assertEquals(
                ...
                js(
                    web,
                    "nodes.some(n=>n.number===2 && n.color==='#74aaff' && n.freshness.includes('Known'))",
                ),
            )
            assertEquals(
                "true",
                js(
                    web,
                    "nodes.some(n=>n.number===3 && n.color==='#5ddac4' && n.freshness.includes('New'))",
                ),
            )
            assertEquals(
                "true",
                js(
                    web,
                    "document.getElementById('legend').textContent.includes('new this discovery')",
                ),
            )
            instrumentation.runOnMainSync {
                assertTrue(
                    "Map retains usable height",
                    web.height >= (160 * panel.resources.displayMetrics.density).toInt(),
                )
            }
            Thread.sleep(1000)
            assertEquals(
                "Discovery markers fit after native controls resize the map",
                "true",
                js(
                    web,
            ...
            if (InstrumentationRegistry.getArguments().getString("captureMap") == "true") {
                ...
                java.io
                    .File(instrumentation.targetContext.cacheDir, "synthetic-discovery-map.png")
                    .outputStream()
                    .use {
            ...
            instrumentation.runOnMainSync {
                ...
                assertFalse(
                    panel.findViewWithTag<android.widget.Switch>("discovery-mode").isChecked
                )
                assertFalse(
                    panel.findViewWithTag<android.widget.Button>("map-survey-start").isEnabled
                )
            }
            assertEquals(
                "true",
                js(web, "nodes.some(n=>n.number===4) && !nodes.some(n=>n.number===3)"),
            )
            instrumentation.runOnMainSync {
                panel.showDiscovery()
                panel.update(
    ...
    private fun js(web: WebView, code: String): String {
        ...
        assertTrue("Map script callback", done.await(5, TimeUnit.SECONDS))
        return answer
    }

    @Test
    fun mapRetainsViewportAndHandlesSelectionOfflineAndTileFailures() {
        ActivityScenario.launch(MainActivity::class.java).use { scenario ->
            lateinit var panel: SatelliteMap
            lateinit var web: WebView
            ...
            scenario.onActivity { a ->
                panel = SatelliteMap(a, { selected.set(it.number) }, {})
                a.setContentView(panel)
                web = panel.findViewWithTag("satellite-map")
                panel.update(state)
            }
            val deadline = System.currentTimeMillis() + 20000
            ...
            assertEquals(
                "true",
                js(web, "document.querySelector('.name').textContent.includes('<node>')"),
            )
            js(web, "map.setView([41,-104],9,{animate:false})")
            val before =
                js(web, "JSON.stringify([map.getCenter().lat,map.getCenter().lng,map.getZoom()])")
            ...
            assertEquals(
                ...
                js(
                    web,
                    "map.getCenter().lat===40 && map.getCenter().lng===-105 && map.getZoom()>=14",
                ),
            )
            assertEquals("true", js(web, "document.querySelector('.node-marker.focused') !== null"))
            assertEquals(
                "true",
                js(web, "document.querySelector('.name').getBoundingClientRect().width>60"),
            )
            ...
            assertEquals(
                "true",
                js(web, "document.getElementById('map').classList.contains('offline')"),
            )
            assertEquals(
                "true",
                js(web, "document.getElementById('notice').textContent.includes('offline')"),
            )
            js(web, "relayUpdate({local:0,online:true,nodes:[]}); imagery.fire('tileerror')")
            assertEquals(
                "true",
                js(web, "document.getElementById('notice').textContent.includes('Reload')"),
            )
            js(web, "relayReload()")
    ...
    @Test
    fun activeSurveyExcludesCacheAndRangeIncludesHoppedReception() {
        ActivityScenario.launch(MainActivity::class.java).use { scenario ->
            lateinit var panel: SatelliteMap
            lateinit var web: WebView
            val now = System.currentTimeMillis()
            ...
            val record =
                SurveyRecord(
                    ...
                    "US",
                    own,
                    listOf(heard, unlocated),
                )
            val state =
                MonitorState(
                    local = 1,
                    nodes = listOf(own, heard, cached, unlocated),
                    surveys = listOf(record),
                )
            scenario.onActivity { a ->
                panel = SatelliteMap(a, {}, {})
                a.setContentView(panel)
                web = panel.findViewWithTag("satellite-map")
                panel.update(state)
            }
            ...
            instrumentation.runOnMainSync {
                ...
                panel.update(
                    state.copy(
                        surveys =
                            listOf(record.copy(id = "new", ended = 0, nodes = emptyList()), record),
                        activeSurvey = "new",
                    )
                )
            }
