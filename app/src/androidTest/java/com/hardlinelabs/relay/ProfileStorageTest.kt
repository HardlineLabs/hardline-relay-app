...
class ProfileStorageTest {
    @Test
    fun protectedQrSurvivesOfflineScanAndKeystoreStorageWithoutUnlocking() {
        ...
        // This fixture is an exact generated bitmap; optical detection is a separate hardware
        // check.
        val decoded =
            MultiFormatReader()
                .decode(
        ...
        try {
            ...
            assertThrows(Exception::class.java) {
                ChannelProfile.open(restored, "wrong password".toCharArray())
            }
            context.contentResolver
                .query(
                    Uri.parse("content://com.hardlinelabs.relay.profiles/profiles"),
                    null,
                    null,
                    null,
                    null,
    ...
    @Test
    fun sharedProfilesKeepRadioActivationAndRemovalIndependent() {
        val context = ApplicationProvider.getApplicationContext<Context>()
        ...
        try {
            store.save(pkg)
            store.selectRadio(1)
            store.activated(pkg, open, 1, 1)
            store.selectRadio(2)
            assertNull(store.active())
            assertFalse(store.switching())
            assertEquals(pkg.id, store.list().single().id)
            store.clearActive()
            assertTrue(store.switching())
            store.selectRadio(1)
            assertEquals(1, store.active()!!.getInt("node"))
            assertFalse(store.switching())
            store.selectRadio(2)
            assertTrue(store.switching())
            ...
            java.io.File(context.noBackupFilesDir, file).delete()
        }
    }
}
