...
class NodeDiscoveryAcceptanceTest {
    @Test
    fun discoveryListensStopsAndPreservesRadioConfiguration() {
        ...
        instrumentation.runOnMainSync {
            context.startForegroundService(Intent(context, RadioMonitorService::class.java))
        }
        ...
        val field =
            RadioMonitorService::class.java.getDeclaredField("mesh").apply { isAccessible = true }
        ...
        try {
            ...
            val known =
                monitor.state.nodes.map { it.number }.toSet() + api.nodes.orEmpty().map { it.num }
            ...
            Thread.sleep(1500)
            assertEquals(
                "Directed survey cannot overlap discovery",
                id,
                monitor.state.activeDiscovery,
            )
            ...
            waitUntil(660_000) {
                monitor.state.events.any { it.survey == id && it.direction == "TX" } ||
                    !monitor.state.surveying
            }
            ...
            assertTrue(
                record.nodes.all {
                    it.observed >= record.started && it.number != monitor.state.local
                }
            )
            assertTrue(record.newNodes.none { it in known })
            assertTrue(
                record.nodes.all { n ->
                    monitor.state.events.any {
                        it.survey == id && it.source == n.id && it.transport == "LoRa"
                    }
                }
            )
            monitor.startDiscovery()
            ...
            Thread.sleep(5000)
            assertEquals(
                "Restart honors the existing request cooldown",
                0,
                monitor.state.surveys.first { it.id == second }.requests,
            )
            monitor.stopSurvey()
            waitUntil(5_000) { !monitor.state.surveying }
