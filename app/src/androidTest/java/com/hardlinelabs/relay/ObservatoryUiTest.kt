...
class ObservatoryUiTest {
    @Test
    fun activeTestBannerReturnsToTheSameSuiteAcrossPages() {
        ActivityScenario.launch(MainActivity::class.java).use { scenario ->
            ...
            scenario.onActivity { a ->
                ...
                state =
                    state.copy(surveying = false, surveyCompletedAt = SystemClock.elapsedRealtime())
                ui.findViewWithTag<Button>("tab-3").performClick()
                assertTrue(
                    ui.findViewWithTag<Button>("test-progress").text.contains("View results")
                )
                ui.destroy()
            }
        }
    ...
    private fun text(view: View): String =
        (if (view is TextView) view.text.toString() else "") +
            if (view is ViewGroup)
                (0 until view.childCount).joinToString("\n") { text(view.getChildAt(it)) }
            else ""

    @Test
    fun packetExpansionAndScrollSurviveLiveArrivals() {
        ActivityScenario.launch(MainActivity::class.java).use { scenario ->
            ...
            scenario.onActivity {
                ...
                feed.update(
                    state.copy(
                        events = events + events.last().copy(time = now + 2, packetId = 1000)
                    )
                )
            }
            instrumentation.waitForIdleSync()
            scenario.onActivity {
                ...
                assertEquals(offset, list.getChildAt(0).top)
                feed.findViewWithTag<Button>("packet-arrivals").performClick()
            }
            instrumentation.waitForIdleSync()
            scenario.onActivity {
                assertEquals(121, list.count)
    ...
    @Test
    fun refreshPreservesPagesAndInspectorControlsTrackTheSelectedNode() {
        ActivityScenario.launch(MainActivity::class.java).use { scenario ->
            ...
            scenario.onActivity { a ->
                ...
                a.setContentView(ui)
                ui.resume()
                radio = ui.findViewWithTag("radio-scroll")
            ...
            Thread.sleep(1200)
            scenario.onActivity {
                assertSame(radio, ui.findViewWithTag("radio-scroll"))
                ui.destroy()
            }
            var starts = 0
            var stops = 0
            ...
            scenario.onActivity { a ->
                suite = NodeInspector(a, 2, {}, { starts++ }, { stops++ }, { focused = it.number })
                a.setContentView(suite)
                suite.update(state)
                assertFalse(suite.findViewWithTag<Button>("node-map").isEnabled)
                suite.update(
                    state.copy(nodes = listOf(node.copy(latitude = 40.0, longitude = -105.0)))
                )
                suite.findViewWithTag<Button>("node-map").performClick()
                assertEquals(2, focused)
                suite.findViewWithTag<Button>("node-test").performClick()
                assertEquals(1, starts)
                state =
                    state.copy(
                        surveying = true,
                        surveyTargets = listOf(2),
                        attempts = listOf(RadioSurvey.Attempt(2, 4, SystemClock.elapsedRealtime())),
                    )
                suite.update(state)
                assertFalse(suite.findViewWithTag<Button>("node-test").isEnabled)
                assertTrue(suite.findViewWithTag<Button>("node-stop").isEnabled)
                suite.findViewWithTag<Button>("node-stop").performClick()
                    ...
                    state.copy(
                        ...
                        attempts =
                            listOf(
                                state.attempts
                                    .single()
                                    .copy(outcome = "Relay acknowledgment", elapsed = 1200)
                            ),
                    )
                )
