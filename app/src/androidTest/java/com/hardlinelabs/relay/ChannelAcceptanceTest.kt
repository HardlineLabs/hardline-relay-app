...
class ChannelAcceptanceTest {
    @Test fun activationAndIndependentRemoval() {
        ...
        val connection = object : ServiceConnection {
            override fun onServiceConnected(name: ComponentName, binder: IBinder) { api = IMeshService.Stub.asInterface(binder); ready.countDown() }
        ...
        try {
            ...
            val active = requireNotNull(store.active()) { "A verified active profile is needed to restore the lab radio." }
            val prior = store.list().first { it.id == active.getString("id") }
            val settings = before.channels.settings[active.getInt("index")]
            check(active.getInt("node") == before.node && active.getInt("slot") == before.config.lora?.channel_num &&
            ...
            val temporary = ChannelProfile.create(ChannelProvisioning.create("QA" + java.util.UUID.randomUUID().toString().take(8)), priorOpen.slot, null)
            ...
            try {
                ...
                assertTrue("Deleting the app profile preserves the radio key", opened.settings == client.read().channels.settings[installed.first])
                store.save(temporary)
                store.clearActive(); Thread.sleep(6000)
                ...
                // Retain a saved profile if a radio write cannot be verified; never conceal an uncertain key.
                if (removalVerified) {
                    val restored = client.activate(priorOpen, before.node) {}
                    store.activated(prior, priorOpen, before.node, restored.first)
                    store.remove(temporary.id)
                    assertTrue("Original channel set restored", before.channels.settings == restored.second.channels.settings)
                    assertTrue("Original RF configuration restored", before.config == restored.second.config)
                }
            }
