...
/**
 * Opt-in two-phone RF comparison. No coordinates, secrets, configuration writes or public-room
 * sends.
 */
class PrivateTransportAcceptanceTest {
    @Test
    fun activateNamedLabProfile() {
        ...
        val connection =
            object : ServiceConnection {
                override fun onServiceConnected(n: ComponentName, b: IBinder) {
                    api = IMeshService.Stub.asInterface(b)
                    ready.countDown()
                }
        ...
        try {
                ...
                report(
                    "Saved profiles: " +
                        available.joinToString {
                            it.name +
                                if (it.locked) " (locked)"
                                else " slot=" + ChannelProfile.open(it, null).slot
                        }
                )
                return
            }
            ...
            report(
                "Verified private profile ${pkg.name}, slot ${opened.slot}, fingerprint ${ChannelProfile.digest(opened.settings.encode()).take(12)}"
            )
        } finally {
    ...
    @Test
    fun compareTextAndPrivateDelivery() {
        ...
        val active =
            requireNotNull(profile.active) {
                "Activate the same private profile on both phones first"
            }
        ...
        assertTrue(
            channel > 0 &&
                settings.psk.size == 32 &&
                !settings.uplink_enabled &&
                !settings.downlink_enabled
        )
        assertEquals(
            "RF slot mismatch; do not transmit",
            requireNotNull(args.getString("expectedSlot")).toInt(),
            before.config.lora?.channel_num,
        )
        val start = requireNotNull(args.getString("start")).toLong()
        val role = requireNotNull(args.getString("role")).toInt()
        require(role in 0..1)
            ...
            object : BroadcastReceiver() {
                ...
                override fun onReceive(c: Context, i: Intent) {
                    runCatching {
                        ...
                        val packet =
                            i.getParcelableExtra<DataPacket>("com.geeksville.mesh.Payload")
                                ?: return
                        if (
                            packet.from == service.myId ||
                                packet.viaMqtt ||
                        ...
                        if (
                            bytes.size < 29 ||
                                !bytes.copyOfRange(0, 4).contentEquals("HLAB".toByteArray())
                        )
                            return
                        ...
                        val seq = text.substring(14, 16).toInt()
                        val mode = seq % 4
        ...
        try {
            report(
                "Ready role=$role slot=${before.config.lora?.channel_num} filter=${before.config.device?.rebroadcast_mode} role=${before.config.device?.role}"
            )
            for (seq in 0 until 8) {
                ...
                assertEquals(before.config, current.config)
                assertEquals(before.channels, current.channels)
                val mode = seq % 4
                val body =
                    "HLAB${start / 1000}${seq.toString().padStart(2, '0')}${System.currentTimeMillis()}"
                        .padEnd(if (mode == 3) 30 else 39, '.')
                val packet =
                    DataPacket(
                        to = DataPacket.ID_BROADCAST,
                        channel = channel,
                        id = service.packetId,
                        bytes = body.toByteArray(Charsets.US_ASCII).toByteString(),
                        dataType = if (mode == 0) 1 else 256,
                        hopLimit = before.config.lora!!.hop_limit,
                        wantAck = mode != 1,
                    )
                service.send(packet)
                report(
                    "TX sample=$seq type=${if (mode == 0) "text" else "private"} reliable=${packet.wantAck} bytes=${body.length}"
                )
            }
            ...
            synchronized(received) {
                report(
                    "RESULT text=${counts[0]}/2 privateUnreliable=${counts[1]}/2 privateReliable=${counts[2]}/2 compactReliable=${counts[3]}/2 rx=${received.size}/8 meanMs=${if (latencies.isEmpty()) -1 else latencies.average().toLong()} maxMs=${latencies.maxOrNull() ?: -1}"
                )
