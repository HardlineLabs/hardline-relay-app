...
class RelayCoreTest {
    @Test fun pinnedAidlConnectionNamesAreMappedExactly() {
    ...
    @Test fun fakeMeshAndAtakCarryPacketsWithoutHardware() {
        val sent = mutableListOf<ByteArray>()
        val received = mutableListOf<ByteArray>()
        val core = BridgeCore(object : MeshTransport {
            override fun send(payload: ByteArray) { sent += payload }
        }, object : AtakTransport {
            override fun receive(payload: ByteArray) { received += payload }
        })
    ...
    @Test fun aes256AuthenticatesPayloadAndContext() {
        ...
        assertThrows(AEADBadTagException::class.java) { Aes256.decrypt(key, intact, byteArrayOf(7)) }
        assertThrows(IllegalArgumentException::class.java) { Aes256.decrypt(key, byteArrayOf(1), context) }
    }

    @Test fun statusMatchesContract() {
        File("../protocol/status-v1.tsv").readLines().filter { !it.startsWith("#") && it.isNotBlank() }
            .map { it.split('\t') }.filter { it[0] == "1" && it[2] != "future_state" }
            .forEach { row ->
                assertEquals(row[3], statusLabel(MeshState.valueOf(row[2].uppercase()), row[1] == "simulated"))
            }
    }
}
